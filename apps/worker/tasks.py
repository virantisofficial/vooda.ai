...
async def _run_ai_triage(db: AsyncSession, job, repo_path: str) -> tuple[int, int, dict[str, int]]:
    ...
    # For local CPU models, enforce a hard asyncio wall-clock timeout so a
    # long-running Ollama generation can't block the queue indefinitely.
    # Cloud providers get 120 s (the BatchTriageConfig default).
